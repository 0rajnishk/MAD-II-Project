<template>
    <main>
        <NavBar />
        <div class="flex">
            <SideBar @play-song="songID" title="Dashboard" />
            <div class="flex-1 p-4 ">
                <div class="left-m">
                    <div class="container mx-auto mt-8 ">
                        <h1 class="text-3xl font-semibold mb-4 text-platinum-white"> playlist: {{ $route.params.id }}
                        </h1>
...
export default {
    ...
    methods: {
        async fetchPlaylists(to) {
            ...
            try {
                const response = await fetch("http://127.0.0.1:8000/api/playlist?id=" + this.$route.params.id, {
                    method: 'GET',
                    headers: {
